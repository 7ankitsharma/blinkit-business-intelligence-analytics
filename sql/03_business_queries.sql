...
SELECT
    ...
    ROUND(
        100*SUM(CASE WHEN sla_met='No' THEN 1 ELSE 0 END)/COUNT(order_id),2
    ) AS breach_rate_pct
FROM orders
WHERE order_status='Delivered'
...
AVG(actual_delivery_minutes) AS avg_speed,
ROUND(
SUM(CASE WHEN sla_met='Yes' THEN 1 ELSE 0 END)/COUNT(*),
4
